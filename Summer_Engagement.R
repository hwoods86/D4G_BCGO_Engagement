...
no_weeks_summer <- att_checkin_Summer_overlap %>%
...
att_engagement_stats <- att_checkin_Summer_overlap %>%
  ...
  # Add number of weeks
  left_join(no_weeks_summer, by = ("check_in_year")) %>%
...
ggplot(plot_data,
  ...
  ggtitle("Summer Engagement Journeys (n = 9036)")
##
#
#Notes Next Steps: checking that only school attendees are included 
#Double check how to verify n